...
Paquete qvalue es una de las opciones que añade al valor p un valor q, el q_value, que se corresponde con un valor p que se incluye en ese nivel de clasificación. Es una variable algo más pequeñita que el valor p.
...
La orden `read.table('gwas.txt')` no da error, pero es errónea. ¿Puedes arreglarla? faltaba indicar la cabecera y también luego indicar que nos representara los 6 primeros datos o filas que ser recogen en los datos.
:::
...
```{r}
ggplot(data = gwas,
       mapping = aes(x = T_STAT / SE, y = P)) +
   geom_point() + 
   geom_abline(intercept = 0, slope = 1, color = 'red')
```

2.  El alelo A1 es siempre el que tiene la frecuencia más baja, en estos datos. Puedo ver este valor de cual es el m´´aximo valor de frecuencia de A1 pidiéndole que me haga la función de densidad acumulada empírica (ECDF) que me da el valor exacto para cada punto. Puedo ver esto respresentado de manera suavuzada pidiendole que me haga geom_density.

```{r}
ggplot(gwas,aes(x = T_STAT, y = P)) +
   geom_line()
ggplot(gwas, aes(x=A1_FREQ, y=T_STAT))+ ggplot(gwas, aes(x=COMITTED, y=T_STAT))
```
...
Los valores p, en caso de ser todas las hipótesis nulas,no hay asociación en ningún caso, la distribución de valore p es uniforme. Hay una mezcla de hipótesis nulas y de hipótesis alternativas, una mezcla homogenea y uniforme y otra que predomina en valores bajos. Si vemos la forma representada en el histograma significa que podemos estimar la proporción o área de snips en los que realmente hay una asociación significativa. En todos estos vamos a estar siempre descubriendo unos cuantos, metiendo un poco de ruido que se corresponde con los falsos positivos. Queremos echarle un ojo a los valores p para ver qué forma tiene esa distribución.
...
Nos estamos dejando muchas asociaciones reales pero que tienen valores de p más grandes. La estrategia es que cuanto menor sea el valor de p escogido, menor proporción de hipótesis nulas con ese valor de p estaré seleccionando por error (menos falsos positivos).
:::
