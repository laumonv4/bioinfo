...
```{r}
taxJSON <- fromJSON(paste0(API, 'taxonomy'))
taxConJSON <- fromJSON(paste0(API, 'taxonomy/uniprot'))
...
::: el motivo por los que hay tantos taxones sin una proteína asignada es `porque entre las proteínas no revisadas han dejado de añadir proteínas muy parecidas a los que tienen otros taxones
```{r}
protTot <- fromJSON(paste0(API,'protein'))[['proteins']][['uniprot']]
protCon <-fromJSON(paste0(API,'protein/structure'))[['proteins']][['uniprot']]
...
```{r}
pfam01530 <- fromJSON(paste0(API,'protein/entry/pfam/PF01530/taxonomy/uniprot/8493'))
pfam01530
```

Los cocodrilos tienen 15 proteínas
:::
